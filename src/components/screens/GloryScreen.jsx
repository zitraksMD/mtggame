...
// VVV AnimatePresence ИМПОРТИРОВАНА (УЖЕ БЫЛА В ВАШЕМ КОДЕ) VVV
...
// XXX ИМПОРТ pageVariants И pageTransition УДАЛЕН СОГЛАСНО ЗАПРОСУ XXX
// import { pageVariants, pageTransition } from '../../animations';
import './GloryScreen.scss'; // Или Achievements.scss

// VVV ВАРИАНТЫ АНИМАЦИИ ДЛЯ КОНТЕНТА ВКЛАДОК (ИЗ ПЕРВОГО СНИППЕТА) VVV
const tabContentVariants = {
    initial: {
        opacity: 0,
        x: 20, // Начинаем немного справа
    },
    animate: {
        opacity: 1,
        x: 0,
    ...
    exit: {
        opacity: 0,
        x: -20, // Уходим немного влево
        position: 'absolute', // Чтобы избежать "прыжка" контента при смене
        width: '100%',    // Сохраняем ширину при exit анимации
        transition: {
            duration: 0.2,
            ease: "easeInOut",
        },
...
const GloryScreen = () => {
    ...
    return (
        <motion.div
            ...
            initial="initial" animate="in" exit="out" // Эти props остаются, могут работать с default Framer Motion transitions
            // variants={pageVariants} transition={pageTransition} // Удалено согласно запросу
        >
            ...
            <div className="tab-content">
                {/* VVV ОБЕРТКА AnimatePresence ДЛЯ АНИМАЦИИ СМЕНЫ ВКЛАДОК VVV */}
                <AnimatePresence mode="wait">
                    {activeTab === 'Trophies' && (
                        <motion.div
                            key="trophies" // Уникальный ключ для AnimatePresence
                            className="trophies-content" // Используем существующий класс из вашего кода
                            variants={tabContentVariants}
                            initial="initial"
                            animate="animate"
                            exit="exit"
                        >
                            {/* СУЩЕСТВУЮЩИЙ КОНТЕНТ ВКЛАДКИ TROPHIES ИЗ ВАШЕГО КОДА */}
                            ...
                            {/* КОНЕЦ СУЩЕСТВУЮЩЕГО КОНТЕНТА ВКЛАДКИ TROPHIES */}
                        </motion.div>
                    )}

                    {activeTab === 'Trials' && (
                        <motion.div
                            key="trials" // Уникальный ключ для AnimatePresence
                            className="trials-content" // Используем существующий класс из вашего кода
                            variants={tabContentVariants}
                            initial="initial"
                            animate="animate"
                            exit="exit"
                        >
                            {/* СУЩЕСТВУЮЩИЙ КОНТЕНТ ВКЛАДКИ TRIALS ИЗ ВАШЕГО КОДА */}
                            ...
                            {/* КОНЕЦ СУЩЕСТВУЮЩЕГО КОНТЕНТА ВКЛАДКИ TRIALS */}
                        </motion.div>
                    )}
                </AnimatePresence>
                {/* ^^^ КОНЕЦ ОБЕРТКИ AnimatePresence ^^^ */}
            </div>
        </motion.div>
    );
};
