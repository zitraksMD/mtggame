...
    // ... (остальные хуки useEffect и функции без изменений до getPopupContent, getPopupTitle, и return) ...
...
    }, [currentChapterId, currentZoneId, setActiveView, setCurrentChapterId, setCurrentZoneId, currentChapterIdFromStore, setCurrentChapterInStore, findZoneIdForChapter]); 

    const RaceDisplay = () => { 
        const playerRace = useGameStore((state) => state.playerRace);
        const [localRace, setLocalRace] = useState(playerRace);
        const currentResetTrigger = useGameStore((state) => state.resetTrigger); 
        useEffect(() => {
            setLocalRace(playerRace);
            console.log("RaceDisplay обновлен, раса:", playerRace);
        }, [playerRace, currentResetTrigger]); 
...
        }, [energyCurrent, energyMax, currentResetTrigger]); 
...
    }, [performViewChange, findZoneIdForChapter]); 
...
            setActivePopup(null);
        }
    }, [resetGame, performViewChange, setCurrentChapterInStore, findZoneIdForChapter]); 

    const handleBattlePassClick = useCallback(() => { if (activeView === 'detailed') setActivePopup('battlepass'); }, [activeView]);
const handleMailClick = useCallback(() => {
    console.log("handleMailClick called. activeView:", activeView); // Для отладки
    if (activeView === 'detailed') {
        setActivePopup('mail');
        console.log("activePopup set to 'mail'"); // Для отладки
    } else {
        console.log("Mail popup not opened because activeView is not 'detailed'.");
    }
}, [activeView]);    const handleDailyGrindClick = useCallback(() => { if (activeView === 'detailed') setActivePopup('hunting');}, [activeView]);
...
    
    // Функция для закрытия любого активного попапа
    const closePopup = useCallback(() => setActivePopup(null), []);

    const getPopupContent = (popupType) => {
        switch (popupType) {
            // case 'mail': // Этот case больше не будет использоваться для рендеринга через стандартный Popup, так как 'mail' обрабатывается отдельно
            //     return <MailPopupContent />; 
            case 'hunting':
                return <div>Содержимое охоты...</div>;
            case 'tasks':
                return <div>Содержимое заданий...</div>;
            case 'exchange':
                return <div>Содержимое обмена...</div>;
            case 'battlepass':
                return <div>Содержимое Battle Pass...</div>;
            default:
                return null;
        }
    };
...
        switch (popupType) { 
            // case 'mail': return "Почта"; // Заголовок для 'mail' теперь отображается в баннере
...
                {/* ... (GlobalMap, ZoneMap, Detailed Chapter View рендерятся здесь без изменений) ... */}
                {activeView === 'global' && (
...
<AnimatePresence>
        {activeView === 'detailed' && activePopup === 'mail' && (
            ...
            >
                <motion.div
                    className="mail-popup-outer-frame" // Этот div теперь в основном для позиционирования баннера и анимации Framer Motion
                    variants={mailPopupFrameVariants}
                    // initial, animate, exit от Framer Motion
                ...
                >
                    ...
                    {/* 2. НОВЫЙ БЛОК для основного синего контента */}
                    <div className="mail-main-content-body"> {/* <<<< ВОТ ЭТОТ БЛОК ПОЛУЧИТ СИНИЙ ГРАДИЕНТНЫЙ ФОН */}
                        
                        {/* Кнопка закрытия теперь внутри синего блока */}
                        <button 
                            onClick={closePopup} 
                        ...
                        {/* MailPopupContent теперь внутри синего блока */}
            ...
            {/* Другие Попапы (если они есть и тоже нуждаются в анимации, их можно обернуть аналогично) */}
            <AnimatePresence>
                {activeView === 'detailed' && activePopup && activePopup !== 'mail' && activePopup !== 'rewards' && (
                    <motion.div 
                        key={activePopup} // Ключ по типу попапа
                        // Здесь можно определить свои variants для стандартных попапов или использовать общие
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 20, transition: {duration: 0.2} }}
                        style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
                        // Оверлей для стандартных попапов можно сделать частью этого motion.div или отдельным
                    >
                        <Popup
