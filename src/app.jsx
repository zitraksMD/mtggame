...
const getChapterIdFromLevelId = (levelId) => {
    ...
    // Если другая логика для ID глав, измени здесь
...
const handleLevelComplete = useCallback((levelId, status, difficultyPlayed) => { // <<< Добавлен difficultyPlayed
    console.log(`🏁 Уровень ${levelId} (сложность: ${difficultyPlayed}) завершён со статусом: ${status}`);
    setActiveLevelData(null); // Сбрасываем данные уровня

    if (status === 'won') {
        // --- Уровень УСПЕШНО пройден ---
        const chapterId = getChapterIdFromLevelId(levelId); // Получаем ID главы
        if (chapterId !== null) {
            // Вызываем action из стора для отметки прохождения уровня на определенной сложности
        ...
        navigate("/main"); // <<< ИЗМЕНЕНО: Переходим на экран наград

    } else if (status === 'lost') {
        // --- Уровень ПРОВАЛЕН (игрок умер) ---
        console.log("Переход в главное меню...");
        navigate("/main"); // Здесь остается переход в главное меню

    } else {
        // --- Другие случаи (например, status === 'error') ---
        console.log("Ошибка уровня или неизвестный статус, переход в главное меню...");
        navigate("/main");
    }
// }, [navigate, setActiveLevelData]); // Старые зависимости
// Добавляем setActiveLevelData, если это setState. navigate уже есть.
// useGameStore.getState() не требует добавления в зависимости.
}, [navigate, setActiveLevelData]); 
