...
const handleLevelComplete = useCallback((levelId, status) => { // <<< Добавили параметры levelId и status
    console.log(`🏁 Уровень ${levelId} завершён со статусом: ${status}`);
    setActiveLevelData(null); // Сбрасываем данные уровня в любом случае
    ...
    if (status === 'won') {
        ...
        console.log("Переход в главное меню...");
        // !!! Убедись, что '/main' - это правильный путь к твоему главному меню !!!
        // Возможно, это просто '/' или другой путь.
        navigate("/main"); // <<< ИЗМЕНЕНО: Переходим в главное меню

    } else {
        // --- Другие случаи (например, status === 'error') ---
        console.log("Ошибка уровня или неизвестный статус, переход в главное меню...");
        navigate("/main"); // По умолчанию тоже в главное меню
    }

// }, [navigate, setActiveLevelData]); // Обновляем зависимости (setActiveLevelData добавлена)
}, [navigate, setActiveLevelData]); // Убедись, что setActiveLevelData здесь доступна, если это setState
